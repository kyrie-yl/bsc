...
type Peer struct {
	...
	*p2p.Peer                    // The embedded P2P package peer
	...
	subVersion SubVersion        // Sub protocol version negotiated
...
func NewPeer(version uint, p *p2p.Peer, rw p2p.MsgReadWriter, txpool TxPool) *Peer {
	...
	p.Log().Info("running broadcast routines", "peer", p.ID().String())
...
// clean it up!
func (p *Peer) Close() {
	p.Log().Info("closing broadcast routines", "peer", p.id)
